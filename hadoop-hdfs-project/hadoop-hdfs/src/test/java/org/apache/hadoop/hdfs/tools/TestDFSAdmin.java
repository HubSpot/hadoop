...
public class TestDFSAdmin {
  ...
  public void testNameNodeGetReconfigurableProperties() throws IOException, InterruptedException {
    ...
    assertEquals(DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_ADAPTIVE_ENABLED, outs.get(11));
    assertEquals(DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_BUSY_RPC_PROCESSING_TIME_MS,
        outs.get(12));
    assertEquals(DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_BUSY_RPC_QUEUE_TIME_MS, outs.get(13));
    assertEquals(DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_HEALTHY_RPC_QUEUE_TIME_MS, outs.get(14));
    assertEquals(DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_MAX_PENDING_LIMIT, outs.get(15));
    assertEquals(DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_MIN_PENDING_LIMIT, outs.get(16));
    ...
    assertEquals(DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_RAMP_DOWN_STEP, outs.get(19));
    assertEquals(DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_RAMP_UP_STEP, outs.get(20));
    assertEquals(DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_SIGNAL_EMA_WINDOW_MS, outs.get(21));
    // Remaining entries (dfs.namenode.heartbeat.recheck-interval onward, plus the
    // port-dependent ipc.<port>.backoff.enable) are left unasserted, as upstream does.
